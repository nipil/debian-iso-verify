#!/usr/bin/env bash

set -u -e -o pipefail


MIRROR=""
ARCH=amd64
TARGET=iso-cd
HASH=SHA256SUMS

TMP=$(mktemp -d)
function cleanup() {
  rm -Rf "${TMP}"

}
trap "cleanup" EXIT

function download_current() {
  curl --fail --silent --show-error --remote-name --output-dir "${TMP}" "http://${MIRROR}/debian-cd/current/${ARCH}/${TARGET}/${HASH}"
  local ISO=$(awk '{print $2}' "${TMP}/${HASH}" | grep '^debian-[^-]*-[^-]*-[^-]*.iso' | head -n 1)
  [[ -n "${ISO}" ]] || { echo "Could not detect ISO name from ${HASH}" >&2; return 1; }
  local VERSION_DL=$(echo "${ISO}" | cut -d '-' -f 2)
  mkdir -p "${VERSION_DL}"
  mv "${TMP}/${HASH}" "${VERSION_DL}"
  [[ -f "${VERSION_DL}/${HASH}.sign" ]] || curl --fail --silent --show-error --remote-name --output-dir "${VERSION_DL}" "http://${MIRROR}/debian-cd/current/${ARCH}/${TARGET}/${HASH}.sign"
  [[ -f "${VERSION_DL}/${ISO}" ]] || curl --fail --silent --show-error --remote-name --output-dir "${VERSION_DL}" "http://${MIRROR}/debian-cd/current/${ARCH}/${TARGET}/${ISO}"
  echo "${VERSION_DL}"
}

function verify_iso() {
  cd "${FOLDER}"
  echo "Verifying signature of hash file..."
  gpg --no-default-keyring --keyring "${KEYRING}" --verify "${HASH}.sign" "${HASH}"
  PROG="${HASH::-1}"  # trim last character
  PROG="${PROG,,}" # convert to lowercase
  echo "Verifying hash of available files..."
  "${PROG}" --check --ignore-missing "${HASH}"
  cd - >/dev/null
}

function build_keyring() {
  # keys taken from https://www.debian.org/CD/verify
  local KEYRING="${TMP}/keys.gpg"

# https://www.debian.org/CD/key-42468F4009EA8AC3.txt
cat <<EOF | gpg --no-default-keyring --keyring "${KEYRING}" --import --quiet
-----BEGIN PGP PUBLIC KEY BLOCK-----


erN2tzzsQRrr685oBUNc19K+PrNj
=KSBQ
-----END PGP PUBLIC KEY BLOCK-----
EOF

# https://www.debian.org/CD/key-988021A964E6EA7D.txt
cat <<EOF | gpg --no-default-keyring --keyring "${KEYRING}" --import --quiet
-----BEGIN PGP PUBLIC KEY BLOCK-----


/p0+6LMNkXLcI6e8MazzVv38JXpXHvzznlZbfKpYQeMLrDXpB4dWHXq0
=zu28
-----END PGP PUBLIC KEY BLOCK-----
EOF

# https://www.debian.org/CD/key-DA87E80D6294BE9B.txt
cat <<EOF | gpg --no-default-keyring --keyring "${KEYRING}" --import --quiet
-----BEGIN PGP PUBLIC KEY BLOCK-----


=h48J
-----END PGP PUBLIC KEY BLOCK-----
EOF

  echo "${KEYRING}"
}


while getopts ":ha:s:t:c:" OPT; do
  case ${OPT} in
    h)
      echo "Usage: ${0} [-a ARCH] [-s SUMS] [-t TARGET] [-c MIRROR_DNS] [FOLDER]" >&2
      exit 0
      ;;
    a)
      ARCH="${OPTARG}"
      echo "Overriding arch ${ARCH}"
      ;;
    s)
      HASH="${OPTARG}"
      echo "Overriding hash ${HASH}"
      ;;
    t)
      TARGET="${OPTARG}"
      echo "Overriding target ${TARGET}"
      ;;
    c)
      MIRROR="${OPTARG}"
      echo "Using mirror ${MIRROR} to download current version"
      ;;
    ?)
      echo "Invalid option: -${OPTARG}." >&2
      exit 1
      ;;
  esac
done
shift $((OPTIND-1))

if [[ -n "${MIRROR}" ]] && [[ ${#} -gt 0 ]]
then
  echo "Cannot fetch current version AND specify a folder to check at the same time" >&2
  exit 1
fi

if [[ -z "${MIRROR}" ]]
then
  FOLDER="${1:-}"
else
  FOLDER=$(download_current)
  echo "Artefacts stored into folder ${FOLDER}"
fi

if [[ -z "${FOLDER}" ]]
then
  echo "No folder specified for verificatoin" >&2
  exit 1
fi

KEYRING=$(build_keyring)

verify_iso
echo "Verification of ${FOLDER} completed"
